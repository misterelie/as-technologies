        ...
        <div class="card">
            <div class="card-header">
                <div class="card-body"><button type="button" class="btn btn-outline-primary" data-toggle="modal"
                        data-target="#exampleModalLogin">Ajouter une marque</button>
                    <!--modal-->
                    ...
                    <tbody>
                        @if(!is_null($marques))
                            @foreach($marques as $marque)
                                <tr>
                                    <td scope="col">{{ $marque->id }}</td>
                                    <td>{{ $marque->designation }}</td>
                                    <td>
                                        <form  id="form-{{$marque->id}}" 
                                            action="{{ url('delete/marque', $marque->id) }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('DELETE')

                                            <a class="btn btn-sm btn-soft-primary btn-circle mr-2" href="#!"
                                                data-target="#modal_{{ $marque->id }}" data-toggle="modal"><i class="dripicons-pencil"
...
<section>
    ...
    @if($marques)
        @foreach($marques as $marque)
            <div class="modal fade" id="modal_{{ $marque->id }}" tabindex="-1" role="dialog"
                aria-labelledby="exampleModalDefaultLogin" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <!--end modal-header-->
                        <div class="modal-body">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active p-3 pt-3" id="LogIn_Tab" role="tabpanel">
                                    <form class="form-horizontal auth-form my-4" method="POST"
                                        action="{{ route('marque.update', $marque->id) }}"
                                        enctype="multipart/form-data">
                                        @csrf
                                        ...
                                        <div class="form-group"><label for="designation">
                                            ...
                                            <div class="input-group mb-3"><input type="text" class="form-control"
                                                    name="designation" id="designation" value="{{ $marque->designation }}"
                                                    value="{{ old('designation') }}"
                                                    placeholder="Veuillez saisir le nom de la marque">
                                            </div>
                                            @error('designation')
